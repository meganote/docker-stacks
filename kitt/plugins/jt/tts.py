...
VOICECLONE_BASE_URL = os.environ.get("VOICECLONE_BASE_URL")
...
class TTS(tts.TTS):
    def __init__(
        ...
        *,
        ...
        encoding: TTSEncoding = "wav_24000_16",
        ...
        self._opts = _TTSOptions(
            ...
            model_id=model_id,
            api_key=api_key,
            base_url=base_url or TTS_BASE_URL,
            encoding=encoding,
...
class ChunkedStream(tts.ChunkedStream):
    ...
    def __init__(self, text: str, opts: _TTSOptions, session: aiohttp.ClientSession) -> None:
        self._opts = opts
        self._text = text
    ...
    def _synthesize_url(self) -> str:
        ...
        # )
        url = f"{VOICECLONE_BASE_URL}/jttts/Voice_Clone"
        return url
    ...
    async def _run(self) -> None:
        async with self._session.post(
            ...
            bytes_per_frame = (self._opts.sample_rate // 100) * 8
            buf = bytearray()
        ...
        async def _forward_events() -> None:
            ...
            while True:
                ...
                async for frame in c.audio_rx:
                    self._event_queue.put_nowait(
                        tts.SynthesisEvent(type=tts.SynthesisEventType.AUDIO, audio=frame)
                    )
        ...
        for try_i in range(max_retry):
            try:
                ...
                if try_i + 1 == max_retry:
                    logger.exception(f"failed to connect to 11labs after {max_retry} retries")
                    return

                retry_delay = min(try_i * 5, 5)  # max 5s
                logger.warning(f"failed to connect to 11labs, retrying in {retry_delay}s")
