...
class LLM(llm.LLM):
    def __init__(
        self,
        *,
        model: str | ChatModels = "jiutian-lan",
        base_url: str | None = None,
        client: jiutianai.AsyncClient | None = None,
    ) -> None:
        self._opts = LLMOptions(model=model)
        self._client = client or jiutianai.AsyncClient()
    ...
    def _to_openai_ctx(self, history: llm.ChatContext):
        ...
        for msg in history.messages:
            ...
            #             {
            #                 "type": "image_url",
            #                 "image_url": {"url": f"data:image/jpeg;base64,{img._cache[self]}"},
            #             }
            #         )
            #     else:
...
class LLMStream(llm.LLMStream):
    def __init__(self, oai_stream: openai.AsyncStream, fnc_ctx: llm.FunctionContext | None) -> None:
        super().__init__()
    ...
    async def __anext__(self) -> llm.ChatChunk:
        ...
        async for chunk in self._oai_stream:
            for i, choice in enumerate(chunk.choices):
                ...
                #                 logger.warning("new fnc call while previous call is still running")
                #             fnc_name = finfo.name
    ...
    async def _call_function(
        ...
        self._called_functions.append(llm.CalledFunction(fnc_name=name, fnc=fnc.fnc, args=args))
...
def image_detail_from_dimensions(width: int, height: int) -> str:
    deltas = [(min(abs(width - d[0]), abs(height - d[0])), d[1]) for d in IMAGE_DETAIL_DIMENSIONS]
